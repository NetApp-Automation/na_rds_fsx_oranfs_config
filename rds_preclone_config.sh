...
LOG=/home/ec2-user/na_rds_fsx_oranfs_config/logs/rds_preclone_config-`date +"%Y-%m%d-%H%M.%S"`.log

echo "Start RDS FSX preclone configuration at `date +"%Y-%m%d-%H%M.%S"`" >> $LOG
...
echo "Execute FSx preclone configuration playbook at `date +"%Y-%m%d-%H%M.%S"` -----> " >> $LOG
...
echo "End RDS FSX preclone configuration at `date +"%Y-%m%d-%H%M.%S"`" >> $LOG
